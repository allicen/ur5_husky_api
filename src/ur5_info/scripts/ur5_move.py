...
from arm_msgs.msg import ManipulatorState, GripperState
...
class Robot():
    def __init__(self, velocity=0.1, acceleration=10.0) -> None:
        ...
        rospy.Subscriber("/joint_states", JointState, self.get_joint_states_gripper)
        ...
        self.state_gripper_pub = rospy.Publisher('/state/arm/0/gripper_state', GripperState, queue_size=10)
    ...
    def get_joint_states_gripper(self, msg):
        ## Publishers: /husky_node (msg.position.size() == 4), /robotiq_2f_action_server (msg.position.size() == 1)
        if len(msg.position) == 1:
            msg_pub = GripperState()
            msg_pub.gripper_joints = msg.position
            msg_pub.gripper_speed = msg.velocity
            self.state_gripper_pub.publish(msg_pub)
